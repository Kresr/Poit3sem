...
char LA::Tokenize(const char* string) {
	FST::FST GRAPHS[] = {FST_ELSE, FST_IF,FST_LEFT_SQUARE_BRACE,FST_RIGHT_SQUARE_BRACE
		...
		FST_PLUS,FST_LEFTBITWISE, FST_RIGHTBITWISE, FST_MINUS, FST_STAR, FST_DIRSLASH, FST_EQUALS,
	...
	const char tokens[] = {LEX_ELSE, LEX_IF, LEX_LEFT_SQUARE_BRACE,LEX_RIGHT_SQUARE_BRACE,
 LEX_STRING, LEX_FUNCTION, LEX_DECLARE,
		...
		LEX_PLUS,LEX_LEFTBITWISE, LEX_RIGHTBITWISE, LEX_MINUS, LEX_STAR, LEX_DIRSLASH, LEX_EQUALS,
...
short LA::Scan(LT::LexTable& lextable, IT::IdTable& idtable, In::IN& in, Parm::PARM& parm, Log::LOG& log) {
	...
	for (int i = 0, line = 1; in.text[i]; ++i) {
		...
		auto fillTables = [&] {
			...
			else if (token == LEX_PRINT)
			{
			...
			else if (token == LEX_ID || token == LEX_POW  || token == LEX_RAND || token == LEX_PRINT) {
				...
				if (ti_idx == TI_NULLIDX) {
					if (lextable.size >= 2 && lextable.table[lextable.size - 2].lexeme == LEX_DECLARE
						...
						(lextable.table[lextable.size - 1].lexeme == LEX_RAND) ||
...
void LA::ShowIDtable(IT::IdTable& idtable, std::ofstream* outfile)
{
	...
	*outfile << std::left // ¬ыравнивание текста по левому краю
		<< std::setw(10) << "[ID]"
		<< std::setw(15) << "[SCOPE]"
		<< std::setw(10) << "[TYPE]"
		<< std::setw(15) << "[IDTYPE]"
	...
	*outfile << "------------------------------------------------------------------------------------------------------------" << std::endl;
	...
	{
		*outfile << std::left
			<< std::setw(10) << ('[' + std::to_string(i) + ']') // »ндекс идентификатора
			<< std::setw(15) << idtable.table[i].scope // ќбласть видимости
			<< std::setw(10);
